<template>
  <div class="grid min-h-screen grid-cols-2 bg-white">
    ...
    <transition name="bounce-left">
      <section
        v-if="ready"
        class="flex items-center justify-center bg-blue-100 border-r border-[#ebeef5]"
      >
        <div class="text-center">
          <img src="../assets/svg/login.svg" alt="" />
          <h1 class="text-3xl font-extrabold">MyDevCraft Admin系統</h1>
          <p class="mt-2 text-gray-600">簡潔、快速、好維護</p>
        </div>
      </section>
    </transition>
    ...
    <transition name="bounce-right">
      <section v-if="ready" class="flex items-center justify-center">
        ...
        <Register
          v-if="isRegister"
          @go-login="isRegister = false"
          @registered="handleRegistered"
        />

        <!-- 原登入區塊（切換條件：非註冊狀態） -->
        <div v-else style="width: 680px; max-width: 90%" class="h-[700px] p-20 pt-32">
